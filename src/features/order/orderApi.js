  ...
  endpoints: (builder) => ({
    ...
    getOrders: builder.query({
      ...
      merge: (currentCache, orders) => {
        ...
        const newCache = currentCache.filter(
          (curr) => curr.__isDirty || orders.some((o) => o.id === curr.id)
        );
        // Remove new orders into current cache if it is not dirty
        const result = mergeRetainDirty(
          newCache,
          orders,
          (curr, o) => curr.id === o.id
        );
...
export const updateOrderAction = (orderId, editedOrderOrFn) =>
  orderApi.util.updateQueryData(
    // Update cache entry with key `getOrders(undefined)`
    "getOrders",
    undefined,
