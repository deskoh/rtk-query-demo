...
const invalidatesTags = (_result, _error, arg) => [
  { type: "OrderItems", id: arg.orderId ?? arg },
];
  ...
  endpoints: (builder) => ({
    ...
    searchItems: builder.query({
      ...
      providesTags: (result, error, { orderId }) =>
        providesId(result, orderId, "OrderItems"),
      ...
      async onQueryStarted({ orderId }, { dispatch, queryFulfilled }) {
        ...
        dispatch(
          saveInitialOrderItemIds({ orderId, itemIds: items.map((i) => i.id) })
        );
      },
    }),
...
export const useSearchItemsQueryState =
  itemApi.endpoints.searchItems.useQueryState;
...
export const addOrderItemAction = (orderId, newItem) => (dispatch) => {
  ...
  // Update parent order
  dispatch(
    updateOrderAction(orderId, (draftOrder) => {
      draftOrder.itemsCount = "...";
    })
  );
};
...
export const useSelectedItem = () => {
  ...
  const selectedItemId = useSelector((state) => state.item.selectedItemId);
  const { data: items } = useSearchItemsQueryState({
